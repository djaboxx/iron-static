"""
iron_static.notify — push notifications and events into VS Code
via the IRON STATIC VS Code Bridge extension (port 9880).

Library usage:
    from iron_static.notify import notify, event, status, progress, open_file, health

CLI (installed as `iron-static-notify` by pyproject.toml):
    iron-static-notify notify "Brainstorm complete" --level info
    iron-static-notify event "done" --source run_brainstorm --type done
    iron-static-notify health

Or via python -m:
    python -m iron_static.notify notify "hello"
"""
...
def event(
    ...
    """
    Push a structured event into Arc's queue via the ironStatic_getEvents LM Tool.
    Arc can call the tool at any time to drain the queue and see what happened.

    source: script name, e.g. "run_brainstorm", "gcs_sync", "vela_vocalist"
    type:   "info" | "warn" | "error" | "done" | "progress"
    """
    payload: dict = {"source": source, "type": type, "message": message}
    if data:
        payload["data"] = data
    return _post("/event", payload)
...
def main() -> None:
    ...
    # event
    p_event = sub.add_parser("event", help="Push an event into Arc's LM Tool queue.")
    p_event.add_argument("message")
    p_event.add_argument("--source", default="cli")
    p_event.add_argument(
        "--type",
        dest="etype",
        choices=["info", "warn", "error", "done", "progress"],
        default="info",
    )
    ...
    if args.cmd == "health":
        ...
        result = event(args.source, args.message, args.etype)
    else:
        parser.print_help()
        sys.exit(1)
